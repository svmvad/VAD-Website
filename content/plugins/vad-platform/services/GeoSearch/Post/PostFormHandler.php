...
class PostFormHandler
{
    ...
    protected function mapMetaFields($feed, $postData, $postType, $formData = [])
    {
        ...
        foreach ($metaInputs as $metaFieldMapping) {
	        ...
	        if (static::maybeRemoveMetaImages($metaFieldMapping['meta_value'])) {
		        $metaFieldMapping['meta_value'] = "";
	        }
            if (
                false !== strpos($metaFieldMapping['meta_value'], 'uploads/fluentform') &&
                $attachmentId = $this->getAttachmentToImageUrl($metaFieldMapping['meta_value'])
            ) {
                $metaFieldMapping['meta_value'] = $attachmentId;
            }
